...
- **ArtifactPruner** reclaims artifact `.zip`s via `deleteArtifact`, keeping only the live release's (plus any in-flight deploy's / not-yet-attached upload); it nulls `storage_path` + stamps `pruned_at` but never deletes the row (DB updated before the file, so a not-yet-migrated schema never strands an orphan). Run by the post-deploy task (§6 step 15b) and the housekeeper (§10). Its `pruneOrphans()` additionally sweeps files the DB-driven pass can't see — `.zip`s no row references (rows pruned before the file was removed, or a deleted project's leftovers) and stale chunked-upload sessions — guarded by a deploy-timeout age so an in-flight upload is never touched.
